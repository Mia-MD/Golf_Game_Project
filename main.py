...
def calcul_de_points(nombre_coups):
    global points_total
    if nombre_coups < 20:
        points = points_total - nombre_coups*5
    else:
        points = 0
...
def trouver_force(xa,xb,ya,yb):
    ...
    if force>200:
        force=200
        print("FORCE MAXIMALE ATTEINTE")
    print("Force de la balle: ", force)
    return force
...
def trouver_angle(xa,xb,ya,yb):
    ...
    compY = -(yb-ya)
    try:
        angle =  math.degrees(math.atan(compY/compX))  #this is good
    except:
        print("oops")
...
def calculForceBalle(force,angle):
...
class Jeu:
    ...
    #https://www.youtube.com/watch?v=ndtFoWWBAoE  source pour l'affichage de texte sur l'ecran
    pygame.font.init()
    ...
    def run(self):
        while Jeu.run:
            ...
            x_obs,y_obs = convert_coordinates(obs_body.position)   
            dx= pygame.mouse.get_pos()[0]-x
            dy = pygame.mouse.get_pos()[1]-y
            Jeu.line = [((x,y)),((x-dx,y-dy))]
            ...
            if Jeu.level_picked:
                Jeu.tracer_ligne(self,Jeu.mouse_drag)
                pygame.draw.circle(self.display_surface, (255, 255, 255), (int(x), int(y)), 7)
            ...
            if Jeu.credits_ouvert:  
                font_credits =  pygame.font.SysFont("Arial ",10)
            ...
            if x > Jeu.x_flag and (y<jeu.y_flag_max and y>Jeu.y_flag_min):
                print("Nombre de coups: ", Jeu.nb_coups, "Points: ", calcul_de_points(Jeu.nb_coups))
                Jeu.enlever_delimitation(Jeu.level_number)
                if Jeu.level_number == 0:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONUP and Jeu.level_picked==True and energie<0.1:
                    ...
                    if tirer == False:  
                        Jeu.nb_coups=Jeu.nb_coups+1
                        pos_x = body.position[0]
                        pos_y=800-body.position[1]
                        pos = pygame.mouse.get_pos()
                        tirer=True
                        force = trouver_force(pos_x,pos[0],pos_y,pos[1])
                        angle = trouver_angle(pos_x,pos[0],pos_y,pos[1])
                        #https://stackoverflow.com/questions/60849981/change-of-impulse-direction-pymunk-python
                        force_balle = calculForceBalle(force,angle)
                        body.velocity=(force_balle[0],force_balle[1])
